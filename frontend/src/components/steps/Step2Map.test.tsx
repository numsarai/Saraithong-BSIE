...
vi.mock('@/api', () => ({
  ...
  evidencePreviewUrl: vi.fn((fileId: string, pageNumber?: number | null) => {
    const suffix = pageNumber ? `#page=${pageNumber}` : ''
    return `/api/files/${encodeURIComponent(fileId)}/evidence-preview${suffix}`
  }),
...
describe('Step2Map analyst gate', () => {
  ...
  it('surfaces OCR account-presence lineage for investigator review', async () => {
    ...
    const previewImage = screen.getByRole('img', { name: /evidence source preview/i })
    expect(previewImage).toHaveAttribute('src', '/api/files/file-1/evidence-preview')
